...
import { requestProducts, requestAddToCart } from "../../redux/modules/product.module";
...
class Home extends Component {
    ...
    render() {
        ...
        if (products.loading || !products.prods) {
            return <Flex justifyContent={"center"} pt={"15%"}> Loading...</Flex>
        }
        return (
            <Flex flex={1} flexDirection={"column"}>
                ...
                <Flex wrap={"wrap"} justifyContent={"space-around"}>
                    {products.prods.map(
                        (item) => {
                            return <ProductCard key={item.title} prodDetail={item} addToCart={this.addToCart}></ProductCard>
                        }
                    )}
